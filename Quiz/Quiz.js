let questions = [
    ...
    },
    {
        ...
        options: ["a) Hoppers (Aapa)", "b) Kottu Roti", "c) Lamprais", "d) Pol Sambol"],
        answer: "2"
    },
    {
...
function handleAnswerClick(event) {
    ...
    const correctAnswerIndex = parseInt(questions[currentQuestion].answer); // Index of the correct answer

    // Remove any previous selections
    for (let i = 0; i < answers.length; i++) {
        answers[i].classList.remove("selected", "correct", "wrong");
    }

    if (selectedAnswerText === questions[currentQuestion].options[correctAnswerIndex]) {
        ...
        for (let i = 0; i < answers.length; i++) {
            if (answers[i].textContent === questions[currentQuestion].options[correctAnswerIndex]) {
